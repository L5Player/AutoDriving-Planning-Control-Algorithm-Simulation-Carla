...
void LqrController::Init() {
    ...
    */
    ...
    // int q_param_size = 4;
    matrix_q_(0, 0) = 0.1;    // TODO: lateral_error
    matrix_q_(1, 1) = 0.001;    // TODO: lateral_error_rate
    matrix_q_(2, 2) = 0.01;    // TODO: heading_error
    matrix_q_(3, 3) = 0.001;    // TODO: heading__error_rate
...
// 两点之间的距离
double PointDistanceSquareLqr(const TrajectoryPointOri &point, const double x, const double y) {
    double dx = point.x - x;
    double dy = point.y - y;
    return dx * dx + dy * dy;
}
...
double atan2_to_PI_LQR(const double atan2) { return atan2 * M_PI / 180; }
...
bool LqrController::ComputeControlCommand(const VehicleState &localization, const TrajectoryData &planning_published_trajectory, ControlCmd &cmd) {
    ...
    */
    ...
    if (steer_angle >= atan2_to_PI_LQR(20.0)) {
        steer_angle = atan2_to_PI_LQR(20.0);
    } else if (steer_angle <= -atan2_to_PI_LQR(20.0)) {
        steer_angle = -atan2_to_PI_LQR(20.0);
    }
...
void LqrController::ComputeLateralErrors(const double x, const double y, const double theta, const double linear_v, const double angular_v, const double linear_a, LateralControlErrorPtr &lat_con_err) {
    TrajectoryPointOri current_closest_point = this->QueryNearestPointByPosition(x, y);
...
// 查询距离当前位置最近的轨迹点
TrajectoryPointOri LqrController::QueryNearestPointByPosition(const double x, const double y) {
    double d_min = PointDistanceSquareLqr(trajectory_points_.front(), x, y);
    size_t index_min = 0;

    for (size_t i = 1; i < trajectory_points_.size(); ++i) {
        double d_temp = PointDistanceSquareLqr(trajectory_points_[i], x, y);
        if (d_temp < d_min) {
            d_min = d_temp;
            index_min = i;
        }
